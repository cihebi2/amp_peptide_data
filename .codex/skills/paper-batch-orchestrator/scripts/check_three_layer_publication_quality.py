#!/usr/bin/env python3
"""Heuristic publication-grade QA for amp_three_layer_v2 batch outputs.

This checker is intentionally stricter than the current structural validator. It
flags semantic/provenance risks that require source-reviewed adjudication.
"""
...
MIC_LIKE = {"MIC", "MBC", "MFC", "IC50", "EC50", "HC50", "CC50", "MHC", "MBIC", "MBEC", "MIC50", "MIC90"}
...
SUSPICIOUS_SPECIES_PATTERNS = [
    re.compile(r"^(?:the|this|these|those|in this|we |our |a |an )\b", re.I),
    re.compile(r"\b(?:constitute|minimal|plot|broad|activity|antimicrobial|database annotation only)\b", re.I),
]
...
def read_json(path: Path) -> Any:
    ...
    except Exception as exc:  # noqa: BLE001 - report parse risk, continue batch audit
        return {"__parse_error__": str(exc), "__path__": str(path)}


def paper_ids_from_manifest(path: Path) -> list[str]:
    data = read_json(path)
    if not isinstance(data, dict) or data.get("__parse_error__"):
        raise SystemExit(f"invalid or missing manifest: {path}")
    ids = data.get("paper_ids") if isinstance(data, dict) else None
...
def has_locator(value: Any) -> bool:
    ...
    if isinstance(value, list):
        return any(has_locator(item) for item in value)
    if not isinstance(value, dict):
        return False
    return any(
        locator_bearing_key(key) and has_locator(item)
...
def target_species(record: dict[str, Any]) -> str:
    ...
    species = target.get("species") or record.get("target_species") or ""
...
def suspicious_species(species: str) -> bool:
    if not species:
        return True
    if any(pat.search(species) for pat in SUSPICIOUS_SPECIES_PATTERNS):
        return True
    if NON_BIOLOGICAL_TARGET_RE.search(species):
        return True
    words = species.split()
    if len(words) > 8 and not re.search(r"\b[A-Z][a-z]+\s+[a-z]{3,}\b", species):
        return True
    return False
...
def explicit_no_unit_rationale(record: dict[str, Any]) -> bool:
    rationale = " ".join(
...
def packet_table_text(root: Path, paper_id: str) -> dict[str, str]:
    path = root / "packets" / paper_id / "extracted" / "pdf_tables.json"
    if not path.exists():
        return {}
...
def issue_log_counts(path: Path | None) -> Counter[str]:
    ...
    for line in path.read_text().splitlines():
        ...
        try:
            ...
            counts["issue_log_parse_error"] += 1
            continue
        typ = str(row.get("issue_type") or "")
        msg = str(row.get("message") or "")
        counts[typ] += 1
        if "accelerator auto-closed" in msg:
            counts["accelerator_auto_closed"] += 1
        if "fallback" in msg or "deterministic" in msg:
            counts["fallback_or_deterministic_event"] += 1
    return counts
...
def main() -> int:
    ...
    parser.add_argument("--allow-risk", action="store_true", help="return exit code 0 even when risks are found")
    args = parser.parse_args()

    root = args.root
    paper_ids = paper_ids_from_manifest(args.manifest)
    ...
    for paper_id in paper_ids:
        ...
        table_text_by_locator = packet_table_text(root, paper_id)
        paths = {
            "review": paper_root / "final" / "review_report.json",
            "activity": paper_root / "final" / "activity_toxicity_evidence.json",
            "mechanism": paper_root / "final" / "mechanism_ontology_record.json",
            "database": paper_root / "final" / "database_record_verification.json",
        }
        for name, path in paths.items():
            if not path.exists():
                risks["missing_final_file"].append({"paper_id": paper_id, "file": name, "path": str(path)})
        if any(not path.exists() for path in paths.values()):
            continue

        review = read_json(paths["review"])
        activity = read_json(paths["activity"])
        mechanism = read_json(paths["mechanism"])
        database = read_json(paths["database"])
        for name, payload in (("review", review), ("activity", activity), ("mechanism", mechanism), ("database", database)):
            if isinstance(payload, dict) and payload.get("__parse_error__"):
                risks["parse_error"].append({"paper_id": paper_id, "file": name, "error": payload["__parse_error__"]})

        review_status[str(review.get("review_status") or review.get("status") or "missing")] += 1
        if str(review.get("summary") or "").strip() == TEMPLATE_SUMMARY:
            risks["templated_review_summary"].append({"paper_id": paper_id})
        if not any(review.get(key) for key in ("reviewed_at", "generated_at", "timestamp")):
            risks["missing_review_timestamp"].append({"paper_id": paper_id})
        if str(review.get("review_model") or review.get("model") or "") != "gpt-5.5":
            risks["missing_gpt55_review_provenance"].append({"paper_id": paper_id})
        if str(review.get("reasoning_effort") or "") != "xhigh":
            risks["missing_xhigh_review_provenance"].append({"paper_id": paper_id})

        strict = review.get("strict_gate") if isinstance(review.get("strict_gate"), dict) else {}
        if int(strict.get("required_rework_count") or 0) > 0 or review.get("rework_targets"):
            risks["open_rework_targets"].append({"paper_id": paper_id, "count": strict.get("required_rework_count")})
        ...
        for index, record in enumerate(activity_records, start=1):
            if not isinstance(record, dict):
                continue
            counters["activity_records"] += 1
            ...
            raw_unit = str(record.get("raw_unit") or "").strip()
            species = target_species(record)
            locator = record_source_locators(record)
            if endpoint.lower() in GENERIC_ENDPOINTS:
                risks["generic_activity_endpoint"].append({"paper_id": paper_id, "row": index, "endpoint": endpoint, "raw_value": raw_value})
            if endpoint.upper() in MIC_LIKE and not raw_unit and not explicit_no_unit_rationale(record):
                risks["mic_like_missing_unit"].append({"paper_id": paper_id, "row": index, "endpoint": endpoint, "raw_value": raw_value})
            if suspicious_species(species):
                risks["suspicious_target_species"].append({"paper_id": paper_id, "row": index, "endpoint": endpoint, "species": species[:160]})
            if not raw_value or not has_locator(locator):
                risks["activity_core_or_locator_gap"].append({"paper_id": paper_id, "row": index, "endpoint": endpoint})
        ...
        for index, record in enumerate(table_evidence_records, start=1):
            if not isinstance(record, dict):
                continue
            locator = record_source_locators(record)
            for table_locator in sorted(table_locator_ids(locator)):
                ...
                if table_text and source_table_is_non_activity(table_text):
                    risks["non_activity_source_table"].append(
                        {
                            "paper_id": paper_id,
                            "row": index,
                            "endpoint": str(record.get("endpoint") or ""),
        ...
        for table_locator, table_text in table_text_by_locator.items():
            if table_has_activity_measurement(table_text) and table_locator not in cited_activity_tables:
                risks["missing_activity_table_coverage"].append(
                    {"paper_id": paper_id, "source_locator": table_locator}
                )
        for mismatch in expected_table_observation_issues(root, paper_id, activity):
            ...
            risks[code].append(
                {"paper_id": paper_id, **{key: value for key, value in mismatch.items() if key != "code"}}
            )
        ...
        for index, claim in enumerate(mechanism.get("mechanism_claims") or [], start=1):
            if not isinstance(claim, dict):
                continue
            counters["mechanism_claims"] += 1
            evidence_class = str(claim.get("evidence_class") or claim.get("mechanism_evidence_class") or "")
            if not claim.get("claim_id"):
                risks["mechanism_missing_claim_id"].append({"paper_id": paper_id, "claim_index": index})
            if not (claim.get("claim_text") or claim.get("summary")):
                risks["mechanism_missing_claim_text"].append({"paper_id": paper_id, "claim_index": index})
            if not has_locator(record_source_locators(claim)):
                risks["mechanism_locator_gap"].append({"paper_id": paper_id, "claim_index": index})
            if evidence_class == "direct_mechanism" and not (claim.get("direct_assay_types") or claim.get("direct_assay_type")):
                risks["direct_mechanism_without_assay"].append({"paper_id": paper_id, "claim_index": index})

    issue_counts = issue_log_counts(args.issues)
    for key in ("accelerator_auto_closed", "fallback_or_deterministic_event", "issue_log_parse_error"):
        if issue_counts.get(key):
            risks[key].append({"count": issue_counts[key]})

    risk_counts: dict[str, int] = {}
    for key, value in sorted(risks.items()):
        if len(value) == 1 and isinstance(value[0], dict) and isinstance(value[0].get("count"), int):
            risk_counts[key] = int(value[0]["count"])
        else:
            risk_counts[key] = len(value)
    ...
    summary = {
        ...
        "issue_log_counts": dict(issue_counts),
        "risk_counts": risk_counts,
        "risk_examples": {key: value[:5] for key, value in sorted(risks.items())},
        "publication_grade_pass": not risks,
    }
    text = json.dumps(summary, ensure_ascii=False, indent=2)
    if args.json_out:
        args.json_out.write_text(text + "\n")
    print(text)
    return 0 if args.allow_risk or not risks else 2
