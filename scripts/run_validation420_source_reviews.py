#!/usr/bin/env python3
"""Run validation420 paper-level source-review Codex jobs with bounded parallelism."""
...
DEFAULT_PACKET_INDEX = ROOT / "reports" / "nar_resource_freeze_v1" / "manual_validation" / "validation420" / "source_review_packets" / "packet_index_latest.csv"
...
def validate_result(path: Path) -> tuple[bool, str, str, int]:
    if not path.exists():
        return False, "missing_result_json", "", 0
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except Exception as exc:  # noqa: BLE001
        return False, f"unreadable_result_json:{type(exc).__name__}", "", 0
    required = ["paper_id", "review_sample_id", "review_model", "reasoning_effort", "final_decision", "sample_row_decisions", "checked_inputs"]
    missing = [key for key in required if key not in data]
    if missing:
        return False, "missing_required_keys:" + ",".join(missing), str(data.get("final_decision", "")), 0
    if data.get("review_model") != "gpt-5.5" or data.get("reasoning_effort") != "xhigh":
        return False, "model_or_reasoning_mismatch", str(data.get("final_decision", "")), len(data.get("sample_row_decisions") or [])
    decision = str(data.get("final_decision", ""))
    if decision not in ALLOWED:
        return False, f"unknown_final_decision:{decision}", decision, len(data.get("sample_row_decisions") or [])
    row_decisions = data.get("sample_row_decisions") or []
    if not isinstance(row_decisions, list) or not row_decisions:
        return False, "missing_sample_row_decisions", decision, 0
    return True, "valid_result_json", decision, len(row_decisions)
...
def run_one(row: dict[str, str], timeout_seconds: int, force: bool) -> dict[str, Any]:
    ...
    if result_path.exists() and not force:
        ok, validation, decision, reviewed_rows = validate_result(result_path)
        status = {
            **row,
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=args.parallel) as pool:
        ...
        for future in as_completed(futures):
            ...
            print(json.dumps({
                "review_sample_id": status.get("review_sample_id"),
    ...
    status_rows.sort(key=lambda row: row.get("review_sample_id", ""))
    csv_path = outdir / f"validation420_source_review_status_{run_id}.csv"
    latest_csv = outdir / "validation420_source_review_status_latest.csv"
    json_path = outdir / f"validation420_source_review_summary_{run_id}.json"
    latest_json = outdir / "validation420_source_review_summary_latest.json"
    ...
    summary = {
        ...
        "selected_sample_rows": sum(int(row.get("sample_count") or 0) for row in status_rows),
        "reviewed_sample_rows": sum(int(row.get("reviewed_row_count") or 0) for row in status_rows),
        "runner_state_counts": dict(Counter(row["runner_state"] for row in status_rows)),
        "validation_counts": dict(Counter(row["runner_validation"] for row in status_rows)),
        "final_decision_counts": dict(Counter(row["final_decision"] for row in status_rows)),
        "outputs": {
