...
You are a new Codex CLI paper-review worker for Batch 4-Team. Re-review exactly one paper: `doi__10.1093_infdis_jiv325`.

## Controller-Safe Execution Guardrails

- Keep the run narrow: read only the worker skill files listed below, `rework_context/doi__10.1093_infdis_jiv325/handoff_context.json`, and the artifact/source paths named in that JSON.
...
- Do not inspect unrelated papers. Your write scope is only `papers/doi__10.1093_infdis_jiv325/`, `paper_packets/doi__10.1093_infdis_jiv325/`, `.miaobi-paper-review/workflows/doi__10.1093_infdis_jiv325/`, and paper-specific `reports/doi__10.1093_infdis_jiv325.*` gate outputs.
- A valid run must finish with a final assistant message. End with a concise final line starting `DONE doi__10.1093_infdis_jiv325` and include one of: `accepted_clean`, `accepted_with_cautions`, `needs_targeted_rework`, or `blocked_missing_primary_material`.
...
- Treat `rework_context/doi__10.1093_infdis_jiv325/handoff_context.json` as the message packet and reopen the source/artifact paths listed there.
...
- Codex owner-worker exited non-zero after bounded review, and strict gates still did not pass; controller retried 2 time(s), tagged the paper for later infrastructure recovery, and advanced. If the worker wrote partial artifacts, the after_worker gate reports above remain the scientific source of truth.
- The framework test inventories real material but does not complete worker-6 source-reviewed adjudication.
- Linked database rows include source_conflict/database-only cases that must be preserved or resolved by source review.
- No parser-supported activity/toxicity rows were extracted; worker must inspect XML/PDF/prose/figures/supplements before acceptance.
...
- packet_manifest: `paper_packets/doi__10.1093_infdis_jiv325/packet_manifest.json`
- locator_index: `paper_packets/doi__10.1093_infdis_jiv325/locators/locator_index.json`
- extraction_status: `paper_packets/doi__10.1093_infdis_jiv325/extraction/extraction_status.json`
- extraction_quality_report: `paper_packets/doi__10.1093_infdis_jiv325/extraction/extraction_quality_report.json`
- analysis_status: `paper_packets/doi__10.1093_infdis_jiv325/analysis/analysis_status.json`
- packet_activity: `paper_packets/doi__10.1093_infdis_jiv325/analysis/activity_toxicity_evidence.json`
- packet_database: `paper_packets/doi__10.1093_infdis_jiv325/analysis/database_record_audit.json`
- packet_mechanism: `paper_packets/doi__10.1093_infdis_jiv325/analysis/mechanism_evidence.json`
- packet_adjudication: `paper_packets/doi__10.1093_infdis_jiv325/analysis/adjudication_report.json`
- rework_requests: `paper_packets/doi__10.1093_infdis_jiv325/rework/rework_requests.jsonl`
- rework_responses: `paper_packets/doi__10.1093_infdis_jiv325/rework/rework_responses.jsonl`
- final_review_report: `papers/doi__10.1093_infdis_jiv325/final/review_report.json`
- final_activity: `papers/doi__10.1093_infdis_jiv325/final/activity_toxicity_evidence.json`
- final_database: `papers/doi__10.1093_infdis_jiv325/final/database_record_verification.json`
- final_mechanism: `papers/doi__10.1093_infdis_jiv325/final/mechanism_ontology_record.json`
- quality_feedback: `papers/doi__10.1093_infdis_jiv325/work/review/quality_feedback.json`
- workflow_context: `.miaobi-paper-review/workflows/doi__10.1093_infdis_jiv325/workflow_context.json`
- state_executions: `.miaobi-paper-review/workflows/doi__10.1093_infdis_jiv325/state_executions.jsonl`
- chat_messages: `.miaobi-paper-review/workflows/doi__10.1093_infdis_jiv325/chat_messages.jsonl`
- agent_logs: `.miaobi-paper-review/workflows/doi__10.1093_infdis_jiv325/agent_logs.jsonl`
- latest_complete_report: `reports/doi__10.1093_infdis_jiv325.complete_message_test_report.json`
...
```bash
python .codex/skills/paper-batch-orchestrator/scripts/semantic_three_layer_gate.py --root . --manifest reports/doi__10.1093_infdis_jiv325.true_rework_queue_manifest.json --json > reports/doi__10.1093_infdis_jiv325.owner_worker.semantic_gate.json
python .codex/skills/paper-batch-orchestrator/scripts/check_three_layer_publication_quality.py --root . --manifest reports/doi__10.1093_infdis_jiv325.true_rework_queue_manifest.json --json-out reports/doi__10.1093_infdis_jiv325.owner_worker.publication_quality.json
```

If the manifest path is absent, create `reports/doi__10.1093_infdis_jiv325.true_rework_queue_manifest.json` containing `{"paper_ids":["doi__10.1093_infdis_jiv325"]}` before running the gates.

## Required Output

1. Repair the owner-layer artifact(s) under the paper-local packet/final/work paths.
2. Update `paper_packets/doi__10.1093_infdis_jiv325/rework/rework_responses.jsonl` with what was checked and what remains.
3. Update `papers/doi__10.1093_infdis_jiv325/work/review/quality_feedback.json` if final QC still fails, with concrete `qc_failure_reasons`.
